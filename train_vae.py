...
data_group.add_argument(
    "--image_folder",
    type=str,
    help="path to your folder of images for learning the discrete VAE and its "
    "codebook",
)
data_group.add_argument(
    "--fake_data",
...
parser.add_argument(
    "--image_size", type=int, required=False, default=128, help="image size"
)
parser.add_argument(
    "--wandb_mode",
    default="online",
...
model_group.add_argument(
    "--smooth_l1_loss", dest="smooth_l1_loss", action="store_true"
)
model_group.add_argument(
...
def init_data(distr_backend: DistributedBackend) -> DataLoader:
    ...
    if isinstance(distr_backend, HorovodBackend) or isinstance(
        distr_backend, XLABackend
    ):
...
def init_vae(distr_backend: DistributedBackend) -> Module:
    ...
    if not isinstance(distr_backend, DeepSpeedBackend) and not isinstance(
        distr_backend, XLABackend
    ):
        vae = vae.cuda()
...
def log_artifact_factory(distr_backend: DistributedBackend) -> Callable:
    ...
    def log_artifact(file_name: str):
        model_artifact = wandb.Artifact(
            "trained-vae", type="model", metadata=model_config
        )
...
def save_model_factory(
    vae: Module, distr_vae: Module, distr_backend: DistributedBackend
) -> Callable:
...
def generate_reconstruction_logs(
    vae: Module, images: torch.Tensor, recons: torch.Tensor
) -> dict:
    ...
    for epoch in range(args.epochs):
        for i, (images, _) in enumerate(distr_dl):
            ...
            loss, recons = distr_vae(
                images, return_loss=True, return_recons=True, temp=temp
            )
            ...
            if distr_backend.is_root_worker():
                ...
                if i % 100 == 0:
                    ...
                    logs.update(
                        generate_reconstruction_logs(vae, images, recons)
                    )
                ...
                if i % 10 == 0:
                    ...
                    print(
                        epoch, i, f'lr - {logs["lr"]:6f} loss - {logs["loss"]}'
                    )
            ...
            if i % 100 == 0:
                ...
                # temperature anneal
                temp = max(
                    temp * math.exp(-args.anneal_rate * global_step),
                    args.temp_min,
                )
